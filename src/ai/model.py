...
2D 랜드마크 좌표에서 1D 합성곱으로 지역 특징을 추출한 뒤,
...
from src.ai.preprocessor import NUM_CLASSES, NUM_COORDS, NUM_FINGERS, NUM_LANDMARKS
...
class GestureCNN(nn.Module):
    ...
    아키텍처:
        1. 1D Conv 블록 (3층): ``(B, 2, 21)`` → 특징 벡터
        2. Finger State MLP: ``(B, 5)`` → 중간 표현
        3. Concat → FC → Softmax 분류

    Args:
        num_classes: 출력 클래스 수 (기본 5).
    ...
    def __init__(
        ...
        # 1D Conv 블록: 입력 (B, C=2, L=21)
        self.conv_block = nn.Sequential(
            # Conv1: (B, 2, 21) → (B, 32, 21)
            nn.Conv1d(
    ...
    def forward(
        ...
        Args:
            landmarks: ``(B, 21, 2)`` 정규화된 2D 좌표 텐서.
            finger_states: ``(B, 5)`` 손가락 상태 힌트 텐서.

        Returns:
            ``(B, num_classes)`` 로짓(logits) 텐서.
        """
        # (B, 21, 2) → (B, 2, 21) for Conv1d
        x = landmarks.permute(0, 2, 1)  # (B, C=2, L=21)
