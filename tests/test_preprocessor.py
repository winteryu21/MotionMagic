...
def _make_fake_landmarks(spread: float = 0.5) -> np.ndarray:
    """테스트용 가짜 21개 관절 좌표 ``(21, 2)`` 생성."""
    rng = np.random.default_rng(42)
    # 손목을 (0.5, 0.5)에 놓고, 다른 관절은 주변에 분포
    base = np.full((21, 2), 0.5, dtype=np.float32)
    offsets = rng.uniform(-spread, spread, size=(21, 2)).astype(np.float32)
    offsets[0] = [0.0, 0.0]  # 손목은 중심
    return base + offsets
...
class TestExtract2DLandmarks:
    """extract_2d_landmarks 함수 테스트."""

    def test_from_dict_list(self) -> None:
        """딕셔너리 리스트에서 2D 좌표 추출."""
        dicts = [{"x": float(i), "y": float(i + 1), "z": 0.0} for i in range(21)]
        result = extract_2d_landmarks(dicts)
    ...
    def test_from_2d_array(self) -> None:
        ...
        result = extract_2d_landmarks(arr_2d)
        assert result.shape == (21, 2)
...
class TestPreprocessSingle:
    ...
    def test_returns_tuple(self) -> None:
        ...
        dicts = [{"x": float(i) * 0.05, "y": float(i) * 0.03, "z": 0.0} for i in range(21)]
        result = preprocess_single(dicts)
        assert result is not None
        landmarks, finger_states = result
        assert landmarks.shape == (21, 2)
